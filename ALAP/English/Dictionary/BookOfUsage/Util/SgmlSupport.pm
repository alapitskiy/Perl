package ALAP::English::Dictionary::BookOfUsage::Util::SgmlSupport;
# Not mine, originally
# package SgmlSupport;

require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(getAttrVal sgml2utf sgml2utf_html utf2sgml pgdp2sgml %ent);
...
BEGIN
{
    ...
    $ent{"varsigma"}    = chr(0x03C2);  #  GREEK SMALL LETTER FINAL SIGMA
    ...
    $ent{"healef"}          = chr(0x5D0);   # з
    $ent{"hebet"}           = chr(0x5D1);   # з
    $ent{"hegimel"}         = chr(0x5D2);   # з
    $ent{"hedalet"}         = chr(0x5D3);   # з
    $ent{"hehe"}            = chr(0x5D4);   # з
    $ent{"hevav"}           = chr(0x5D5);   # з
    $ent{"hezayin"}         = chr(0x5D6);   # з
    $ent{"hehet"}           = chr(0x5D7);   # з
    $ent{"hetet"}           = chr(0x5D8);   # з
    $ent{"heyod"}           = chr(0x5D9);   # з
    $ent{"hefinalkaf"}      = chr(0x5DA);   # з
    $ent{"hekaf"}           = chr(0x5DB);   # з
    $ent{"helamed"}         = chr(0x5DC);   # з
    $ent{"hefinalmem"}      = chr(0x5DD);   # з
    $ent{"hemem"}           = chr(0x5DE);   # з
    $ent{"hefinalnun"}      = chr(0x5DF);   # з
    $ent{"henun"}           = chr(0x5E0);   # з
    $ent{"hesamekh"}        = chr(0x5E1);   # зЁ
    $ent{"heayin"}          = chr(0x5E2);   # зЂ
    $ent{"hefinalpe"}       = chr(0x5E3);   # зЃ
    $ent{"hepe"}            = chr(0x5E4);   # зЄ
    $ent{"hefinaltsadi"}    = chr(0x5E5);   # зЅ
    $ent{"hetsadi"}         = chr(0x5E6);   # зІ
    $ent{"heqof"}           = chr(0x5E7);   # зЇ
    $ent{"heresh"}          = chr(0x5E8);   # зЈ
    $ent{"heshin"}          = chr(0x5E9);   # зЉ
    $ent{"hetav"}           = chr(0x5EA);   # зЊ

    $ent{"hesheva"}         = chr(0x5B0);   # жА
    $ent{"hehatafsegol"}    = chr(0x5B1);   # жБ
    $ent{"hehatafpatah"}    = chr(0x5B2);   # жВ
    $ent{"hehatafqamats"}   = chr(0x5B3);   # жГ
    $ent{"hehiriq"}         = chr(0x5B4);   # жД
    $ent{"hetsere"}         = chr(0x5B5);   # жЕ
    $ent{"heseqol"}         = chr(0x5B6);   # жЖ
    $ent{"hepatah"}         = chr(0x5B7);   # жЗ
    $ent{"heqamats"}        = chr(0x5B8);   # жИ
    $ent{"heholam"}         = chr(0x5B9);   # жЙ

    $ent{"hequbuts"}        = chr(0x5BB);   # жЛ
    $ent{"hedagesh"}        = chr(0x5BC);   # жМ

    $ent{"hemaqaf"}         = chr(0x5BE);   # жО

    $ent{"heshindot"}       = chr(0x5C1);   # з
    $ent{"hesindot"}        = chr(0x5C2);   # з
    ...
    $ent{"uumlcirc"}    = chr(0x00FC) . chr(0x0302); # u with diaresis and circumflex                                                               O
