...
use constant {
  ...
  OUT_FILE => 'D:/link/tempf/amBook/amBook.dsl',
};
...
#    ($key, $value) = map {encode("utf-8", decode_entities($_))} ($key, $value);
    eval {
      ($key, $value) = map {Encode::decode("windows-1252", decode_entities($_))} ($key, $value);
    };
    if (my $err = $@) {
      say "ERROR while decoding for $key";
      say "ER: $@";
    }

    use ALAP::English::Dictionary::BookOfUsage::Util::SgmlSupport;
    $value =~ s!\[g\].*?\/([^\/]*)\.gif\[\/g\]!$ent{$1}!g;
...
#    say "VALUE\n$val";
  ...
  say encode("utf-8", decode_entities($_)) for sort keys %h;
